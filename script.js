function searchGoogleBooks() {
    ...
    fetch('https://www.googleapis.com/books/v1/volumes?q=' + txtSearch.value + "&startIndex=0&maxResults=30")
        ...
        .then(json => {
            ...
            for (let i = 0; i < json.items.length; i++) {
                ...
                template += `<li class="list-group-item">Author: ${bookInfo.authors}</li>`
                template += `<li class="list-group-item">Publisher: ${bookInfo.publisher}, Published Date: ${bookInfo.publishedDate}</li>`
                template += `</ul>`
                list += template;
            }
            list += `</div>`;
